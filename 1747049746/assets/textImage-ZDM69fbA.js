import{ca as l,cb as u,cc as h,cd as f,ce as p,cf as v,cg as T}from"./index-BVho6Smp.js";class k{constructor(){this.size=6,this.waitingTaskList=[],this.executingTaskCount=0,this.resolveFunction=t=>{},this.rejectFunciton=()=>{}}execute(t){return t.length===0?Promise.resolve():new Promise(s=>{this.resolveFunction=s,this.waitingTaskList=t,this.check()})}check(){this.executingTaskCount<this.size&&this.waitingTaskList.length>0&&(this.executingTaskCount+=1,this.waitingTaskList[0].call({}).then(()=>{this.executingTaskCount-=1,this.waitingTaskList.length===0&&this.executingTaskCount===0?this.resolveFunction():this.check()}).catch(t=>{throw this.rejectFunciton(),new Error(t)}),this.waitingTaskList.shift(),this.check())}}let n={};async function L(e){try{await l(e,e.textLayerMap),n={};const t=Object.keys(e.textLayerMap),s=new Date().getTime();await new k().execute(t.map(a=>()=>{const c=e.textLayerMap[a];try{return $(e,c)}catch{throw new Error("Failed to create image")}})),u([`Cost ${((new Date().getTime()-s)/1e3).toFixed(2)}s for creating ${t.length} images.`])}catch{throw new Error("Failed to create images")}}function b(e){return n[e].img||null}async function $(e,t){function s(g){return t.textData.characters.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/&/g,"&amp;")}const a=`<div style="${h(f())}">${s(t.textData.characters)}</div>`,c=`<div style="${h(p(t))}">${s(t.textData.characters)}</div>`;n[t.id]||(n[t.id]={status:"",contentHash:"",img:new Image});const i=n[t.id],d=[t.width,t.height,t.fontName.fullname,a,c].join("|");i.contentHash=d;const m=await v(e,t,t.fontName.fullname),o=`
  ...
  </svg>`;if(u(["Created image",{text:t.textData.characters,src:o}]),i.status==="loading")return"";const{img:r}=i;return r.setAttribute("crossOrigin","Anonymous"),r.src=T(o),new Promise((g,w)=>{r.onload=()=>{i.status="loaded",g("")},r.onerror=x=>{throw i.status="",console.error(x),new Error(`Failed to create image. Possibly because there is illegal characters in the text: "${t.textData.characters}"`)}})}export{L as c,b as g};
